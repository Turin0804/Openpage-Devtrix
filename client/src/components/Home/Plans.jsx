        ...
        <section className="bg-zinc-900 border-y border-white/[0.06] py-16 sm:py-20">
            <Container>
                <div className="text-center mb-10 sm:mb-14">
                    <p className="text-orange-400 text-xs font-semibold uppercase tracking-widest mb-3">Pricing</p>
                    ...
                    {plans.map((plan, index) => (
                        <div
                            ...
                            className={`relative rounded-2xl p-6 sm:p-7 flex flex-col gap-5 transition-all duration-300 hover:-translate-y-1 ${
                                plan.popular
                                    ? "bg-gradient-to-b from-orange-600 to-orange-700 border border-orange-500/60 shadow-2xl shadow-orange-900/40"
                                    : "bg-zinc-800 border border-zinc-700 hover:border-orange-500/30"
                            }`}
                        >
                            {plan.popular && (
                            ...
                            <div>
                                <p className={`text-xs font-semibold uppercase tracking-widest ${plan.popular ? "text-orange-200" : "text-orange-400"}`}>
                                    {plan.label}
                                </p>
                                <h3 className={`text-xl font-bold mt-1 ${plan.popular ? "text-white" : "text-white"}`}>
                                    {plan.subscriptionPeriod}
                                </h3>
                                <p className={`text-sm mt-1 ${plan.popular ? "text-orange-100/80" : "text-gray-500"}`}>{plan.desc}</p>
                            </div>

                            <div className={`text-4xl font-bold ${plan.popular ? "text-white" : "text-orange-400"}`}>
                                ${plan.amount}
                                <span className={`text-sm font-normal ml-1 ${plan.popular ? "text-orange-100/70" : "text-gray-500"}`}>one-time</span>
                            </div>

                            <ul className="space-y-2.5 flex-grow">
                                {features.map((f, i) => (
                                    <li key={i} className={`flex items-center gap-2.5 text-sm ${plan.popular ? "text-orange-100" : "text-gray-400"}`}>
                                        <FiCheck size={14} className={plan.popular ? "text-white flex-shrink-0" : "text-orange-400 flex-shrink-0"} />
                                        {f}
                                    </li>
                                ))}
                            </ul>
